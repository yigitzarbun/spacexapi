...
function Future(props) {
  ...
  if (upcoming === null || crew === null) {
    ...
    for (let p = 0; p < upcoming.length; p++) {
      if (upcoming[p]["crew"].length > 0) {
        for (let k = 0; k < upcoming[p]["crew"].length; k++) {
          let member = crew.find(
            (m) => m.id === upcoming[p]["crew"][k]["crew"]
          );
          if (member != null && member != undefined) {
            crewMembers.push({
              ...
              role: upcoming[p]["crew"][k]["role"],
              member_id: member.id,
              launch_id: upcoming[p]["id"],
            });
          }
        }
      }
